...
public class ResourceAnnotationsUtilTestCase {
    ...
    @Test
    public void testAnnotationsWithAllNullFieldsAddsNothing() {
        JsonObjectBuilder builder = Json.createObjectBuilder();
        ResourceAnnotationsUtil.addAnnotations(builder, new Annotations(null, null));
        JsonObject result = builder.build();
        assertFalse(result.containsKey("annotations"));
    }
    ...
    public void testAudienceOnly() {
        JsonObjectBuilder builder = Json.createObjectBuilder();
        ResourceAnnotationsUtil.addAnnotations(builder, new Annotations("user", null));
        JsonObject result = builder.build();
    ...
    public void testPriorityOnly() {
        JsonObjectBuilder builder = Json.createObjectBuilder();
        ResourceAnnotationsUtil.addAnnotations(builder, new Annotations(null, 0.8));
        JsonObject result = builder.build();
        assertTrue(result.containsKey("annotations"));
        JsonObject annotations = result.getJsonObject("annotations");
    ...
    public void testBothAudienceAndPriority() {
        JsonObjectBuilder builder = Json.createObjectBuilder();
        ResourceAnnotationsUtil.addAnnotations(builder, new Annotations("assistant", 0.5));
        JsonObject result = builder.build();
        assertTrue(result.containsKey("annotations"));
        JsonObject annotations = result.getJsonObject("annotations");
